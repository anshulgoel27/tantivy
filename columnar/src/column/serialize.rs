...
pub fn open_column_bytes(file_slice: FileSlice, format_version: Version) -> io::Result<BytesColumn> {
    let (body, dictionary_len_bytes) = file_slice.split_from_end(4);
    let dictionary_len = u32::from_le_bytes(dictionary_len_bytes.read_bytes()?.as_slice().try_into().unwrap());
